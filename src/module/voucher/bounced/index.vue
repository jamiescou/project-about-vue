      ...
      <!-- <el-row  style="">
        <el-col :span='24'>
          <el-button-group  class="ml18">
            <el-button type="primary"  size='small' icon="null iconfont icon-shanchu" :disabled="selected.length==0" @click="deleteVoucher()">删除</el-button>
          </el-button-group>
        </el-col>
      </el-row> -->
...
const base = {
  ...
  methods: {
    ...
    applyApproval () {
      ...
      coreCompanyVoucherService.returnedVoucher.remote(ids, (result) => {
        if (result.success) {
          this.getGrid()
        } else {
    ...
    getGrid () {
      ...
      coreCompanyVoucherService.returnedVoucher.remote(params, (result) => {
        if (result.success) {
